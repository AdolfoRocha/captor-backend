...
// POST finalize-scraping - Force transition from scraping to messaging
// Sets targetLimit to current scraped count, stopping any new scrapes
missionsRouter.post('/:id/finalize-scraping', async (req, res) => {
    try {
        const { id } = req.params;

        // Count how many targets are ready for messaging or already done
        ...
        console.log(`📊 Finalize scraping for mission ${id}: ${successCount} targets ready`);

        // Update mission targetLimit to this count
        const mission = await prisma.mission.update({
            where: { id },
            data: {
                targetLimit: successCount
            }
        });
        ...
        res.status(500).json({ error: 'Failed to finalize scraping' });
    }
});
